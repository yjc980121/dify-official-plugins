...
class DingtalkBotTool(Tool):
    def _invoke(
        ...
        picUrl: str = tool_parameters.get("picUrl")
        messageUrl: str = tool_parameters.get("messageUrl")
        ...
        payload = {"msgtype": msgtype, "link": {"text": content, "title": title, "picUrl": picUrl, "messageUrl": messageUrl}}
        try:
            res = httpx.post(api_url, headers=headers, params=params, json=payload)
            if res.is_success:
                yield self.create_text_message("Link message sent successfully")
            else:
                yield self.create_text_message(
                    f"Failed to send the link message, status code: {res.status_code}, response: {res.text}"
                )
