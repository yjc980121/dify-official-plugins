...
class DingtalkBotTool(Tool):
    def _invoke(
        ...
        payload = {"msgtype": msgtype, "markdown": {"text": content,"title": title}, "at": {}}
        if atMobiles:
        ...
        try:
            ...
            if res.is_success:
                yield self.create_text_message("Markdown message sent successfully")
            else:
                yield self.create_text_message(
                    f"Failed to send the markdown message, status code: {res.status_code}, response: {res.text}"
                )
